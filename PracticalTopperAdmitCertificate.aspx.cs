...
public partial class PracticalTopperAdmitCertificate : System.Web.UI.Page
{
    DBHelper dl = new DBHelper();

    private static readonly log4net.ILog log = log4net.LogManager.GetLogger(typeof(PracticalTopperAdmitCertificate));
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (!IsPostBack)
        {
            ...
            try
            {
                ...
                string studentDataRaw = string.Empty;

                // ✅ Read data directly from Session
                if (Session["StudentAdmitCardData"] != null)
                {
                    studentDataRaw = Session["StudentAdmitCardData"].ToString();

                    // Optional: remove session after reading
                    Session.Remove("StudentAdmitCardData");

                    log.Info("Student data loaded from Session.");
                }
                else
                {
                    log.Warn("Session StudentAdmitCardData is null or expired.");
                }

                if (string.IsNullOrEmpty(studentDataRaw))
                {
                    log.Warn("No student data found. Nothing to render.");
                    return;
                }

                // ✅ Process student data
                List<string> selectedStudents = studentDataRaw
                    .Split(new string[] { ",|" }, StringSplitOptions.RemoveEmptyEntries)
                    .ToList();

                log.Info(string.Format("Total students to process: {0}", selectedStudents.Count));
                ...
                foreach (string studentEntry in selectedStudents)
                {
                    ...
                    if (parts.Length != 4)
                    {
                        log.Warn(string.Format("Skipping invalid entry (expected 4 parts): '{0}'", studentEntry));
                        continue;
                    }

                    string studentIdStr = parts[0].Trim();
                    string collegeId = parts[1].Trim();
                    string facultyId = parts[2].Trim();
                    string examtypid = parts[3].Trim();

                    int studentId;
                    if (!int.TryParse(studentIdStr, out studentId))
                    {
                        log.Warn(string.Format("Invalid studentId format: '{0}'", studentIdStr));
                        continue;
                    }

                    log.Debug(string.Format("Processing studentId={0}, collegeId={1}, facultyId={2}, examTypeId={3}",
                        studentId, collegeId, facultyId, examtypid));

                    DataTable dtPersonalDetails = dl.GetStudentDummyExamCertificateData(
                        studentId,
                        Convert.ToInt32(collegeId),
                        Convert.ToInt32(facultyId));

                    // ⚠️ Theory page  → pass false as last param
                    // ⚠️ Practical page → pass true  as last param
                    DataTable dtSubjects = dl.GetStudentExamAddmitCardSubjectDetails(
                        studentId,
                        Convert.ToInt32(collegeId),
                        Convert.ToInt32(facultyId),
                    ...
                    foreach (DataColumn col in dtPersonalDetails.Columns)
                    {
                        ...
                        if (col.ColumnName == "StudentPhotoPath")
                        {
                            string photo = studentRow[col] != DBNull.Value ? studentRow[col].ToString() : "";
                            combinedRow["StudentPhotoPath"] = string.IsNullOrEmpty(photo)
                                ? (object)DBNull.Value
                                : "~/Uploads/StudentsReg/Photos/" + photo;
                        }
                        else if (col.ColumnName == "StudentSignaturePath")
                        {
                            string sig = studentRow[col] != DBNull.Value ? studentRow[col].ToString() : "";
                            combinedRow["StudentSignaturePath"] = string.IsNullOrEmpty(sig)
                                ? (object)DBNull.Value
                                : "~/Uploads/StudentsReg/Signatures/" + sig;
                        }
                        else
                        {
                            combinedRow[col.ColumnName] = studentRow[col] == DBNull.Value
                                ? DBNull.Value
                                : studentRow[col];
                        }
                    }
                    ...
                    CategorizeAndPopulateSubjects(dtSubjects, combinedRow, Convert.ToInt32(facultyId), collegeId);
                    finalStudentData.Rows.Add(combinedRow);
                }
                ...
                if (finalStudentData.Rows.Count > 0)
                {
                    rptStudents.DataSource = finalStudentData;
                    rptStudents.DataBind();
                    log.Info(string.Format("Successfully bound {0} student rows to repeater.", finalStudentData.Rows.Count));
                }
                else
                {
                    log.Warn("No valid student rows were processed.");
                }
            }
            catch (Exception ex)
            {
