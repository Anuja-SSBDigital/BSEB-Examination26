...
public class DBHelper
{
    ...
    public DataSet GetExmPaymntDetailsTxnIdwise(string ClienTxnId)
    {
        DataSet ds = new DataSet();
        try
        {
    ...
    public DataTable GetCollegeWiseEXMSeatSummary(int CollegeId, int facultyid)
    {
        DataTable dt = new DataTable();
        try
        {
...
string matrixBoardId, string category, string differentlyAbled)
    ...
    public DataTable getCasteCategory_Mstfordropdown()
    {
        ...
        try
        {
...

            {
                ...
                string query = "SELECT PK_CasteCategoryId,CasteCategoryName as CasteCategoryCode FROM CasteCategory_Mst where IsActive='1' order by CasteCategoryName ASC";
    ...
    public bool UpdateCollegeDetails(int collegeId, string principalName, string principalMobile, string email)
    {
        try
        {
            using (SqlConnection conn = new SqlConnection(connectionString))
    ...
    public bool UpdateUserPassword(string userName, string hashedPassword)
    {
        try
        {
            string query = "UPDATE CollegeUsers SET [Password] = @Password, UpdatedDate = GETDATE() WHERE UserName = @UserName";
    ...
    public DataTable getExamDwnldStudentData(string CollegeId, string CollegeCode, string StudentName, int facultyId, int ExamId, string CategoryName, string subCategory)
    {
        DataTable dt = new DataTable();
    ...
    public int UpdateStudentExamRegForm(int studentId, string name, string father, string mother, string dob, string mobile, string email, int casteCategoryId, int genderId, int ExamTypeId, string AaparId)
    {
        try
        {
            using (SqlConnection conn = new SqlConnection(connectionString))
            {
                using (SqlCommand cmd = new SqlCommand("sp_UpdateStudentExamRegForm", conn))
                {
                    cmd.CommandType = CommandType.StoredProcedure;
                    ...
                    cmd.Parameters.AddWithValue("@CasteCategoryId", casteCategoryId == 0 ? (object)DBNull.Value : casteCategoryId);
                    cmd.Parameters.AddWithValue("@GenderId", genderId == 0 ? (object)DBNull.Value : genderId);
                    cmd.Parameters.AddWithValue("@ExamTypeId", ExamTypeId == 0 ? (object)DBNull.Value : ExamTypeId);

                    cmd.Parameters.AddWithValue("@ApaarId", AaparId ?? (object)DBNull.Value);
                    conn.Open();

                    object result = cmd.ExecuteScalar();
                    int studentIdResult;
                    return (result != null && int.TryParse(result.ToString(), out studentIdResult)) ? studentIdResult : 0;
                    conn.Close();
                }
            }
        }
        ...
        {
            log.Error("Error in UpdateStudentExamRegForm: ", ex);
            throw;
        }
    }

    public DataTable GetExamStudentRegiListData(int CollegeId, int facultyId, int ExamId, string CategoryName, string StudentName)
    {
        DataTable dt = new DataTable();
    ...
    public DataTable GetStudentDummyadmitData(string CollegeId, string facultyId)
    {
    ...
    public DataTable GetExamSubjectsByGroup(int FacultyId, int CollegeId, int ExamTypeId, int StudentId)
    {
        DataTable dt = new DataTable();
        try
        {
    ...
    public string InsertStudentUploadHistory(int studentId, string photoPath, string signPath)
    {
        ...
        string query = @"
        INSERT INTO StudentUploadHistory (Fk_StudentId, photoPath, signPath)
        VALUES (@StudentId, @photoPath, @signPath)";
        ...
        try
        {
            using (SqlConnection conn = new SqlConnection(connectionString))
            {
                using (SqlCommand cmd = new SqlCommand(query, conn))
                {
                    ...
                    cmd.Parameters.AddWithValue("@signPath", signPath);
