<%@ Page Language="C#" AutoEventWireup="true" MasterPageFile="~/MasterPage.master" CodeFile="ExamCorrectionForm.aspx.cs" Inherits="ExamCorrectionForm" %>
 ...
 <script type="text/javascript">
    ...
    // Mother Name
           ...
           <%--  var oldStudentName = document.getElementById('<%= hfOldStudentName.ClientID %>').value.trim();
            ...
            // Get new values
            var newStudentName = document.getElementById('<%= txtStudentName.ClientID %>').value.trim();
            var newFatherName  = document.getElementById('<%= txtfatherName.ClientID %>').value.trim();
            var newMotherName  = document.getElementById('<%= txtmotherName.ClientID %>').value.trim();
                     var newDOB = document.getElementById('<%= txtDOB.ClientID %>').value.trim();

             // Track changed fields
             var changedFields = [];

             if (newStudentName !== oldStudentName) changedFields.push("Student Name");
             if (newFatherName !== oldFatherName) changedFields.push("Father Name");
             if (newMotherName !== oldMotherName) changedFields.push("Mother Name");
             if (newDOB !== oldDOB) changedFields.push("Date of Birth");

             if (changedFields.length > 1) {
                 swal("Error",
                     "You are allowed to update only 1 field among Student Name, Father Name, Mother Name, and Date of Birth.\n\nYou changed: "
                     + changedFields.join(", "),
                     "error");
                 return false;
             }--%>
...
function (response) {
    ...
    const lockedFields = [
        ...
        '<%= txtrollNumber.ClientID %>'
         ...
         // Fields to unlock in examType 2, 3, 4, 6
         const unlockedFieldsForType2346 = [
        '<%= ddlGender.ClientID %>',
        '<%= ddlCasteCategory.ClientID %>',
        '<%= ddlNationality.ClientID %>',
        '<%= ddlReligion.ClientID %>',
        '<%= ddlMaritalStatus.ClientID %>',
        // You can add more fields that need to be unlocked for these exam types here
    ];

    // Collect all inputs, selects, textareas, divs
    const allFields = document.querySelectorAll('input, select, textarea, div');

    // --- CASE 1: ExamType = 1 (All fields unlocked except specific ones) ---
    if (examType === "1") {
        allFields.forEach(function (field) {
            if (field.id === '<%= btnUpdate.ClientID %>') return; // skip update button
    ...
    // --- CASE 2: ExamType = 2, 3, 4, 6 (Unlock only specific fields) ---
    if (["2", "3", "4", "6"].includes(examType)) {
        allFields.forEach(function (field) {
            if (field.id === '<%= btnUpdate.ClientID %>') return; // skip update button
            ...
            // Unlock the specific fields for these exam types
            if (unlockedFieldsForType2346.includes(field.id)) {
                field.disabled = false;
            }
        });
